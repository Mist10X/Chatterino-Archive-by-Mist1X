...
from PySide6.QtWidgets import QLineEdit,QPlainTextEdit,QCheckBox,QListWidget
...
class RewardDialog(Dialog):
    def __init__(self,app):
        super().__init__(app,'Награды: мут и анмут','Сначала покупка с ником, затем сообщение указанного бота и фактическое наказание или снятие мута. Неоднозначные совпадения остаются без подписи покупателя.')
    ...
    def refresh(self):
        self.saved.clear()
        for r in self.rules:self.saved.addItem('#'+r['channel']+' · '+('Мут' if r['action']=='timeout' else 'Анмут')+' · '+r['bot'])
    def select(self,i):
    ...
    def save(self):
        def work():
            ...
            if not r['reward'] or not r['bot']:raise ValueError('Выбери полученную награду и укажи ник бота.')
